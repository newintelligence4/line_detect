...
def wrapping(image):
    ...
    _image=cv2.wrapPerspective(image,transform_matrix,(w,h))
...
ret, tresh=cv2.theshold(_gray,160,255,cv2.THRESH_BINARY)
...
def slide_window_search(binary_warped, left_current):
    out_image=np.dstack((binary_warped, binary_warped,binary_warped))

    nwindows=4
    window_height=np.int((binary_warped, binary_warped, binary_warped))
    ...
    for w in range(nwindows):
        ...
        win_xright_low=left_current-margin
        win_xright_high=left_current+margin
    ...
    right_lane=np.concatenate(rate_lane)
...
def draw_lane_lines(original_image,wraped_image,Minv,draw_info):
    ...
    wrap_zero=np.zero_like(wraped_image).astype(np.unit8)
    ...
    cv2.fillPoly(color_wrap,np.int_([pts_mean]),(216,168,74))
